...
seoTitle: "I Built a Full-Stack AI App on Cloudflare Workers With D1, Durable Objects, and Queues — Here's What Actually Worked"
seoDescription: "I built clickfolio.me entirely on Cloudflare's free tier: D1, R2, Queues, Durable Objects, Gemini AI. $0 hosting, 50ms response times. Every technical decision explained."
coverAlt: "Architecture diagram of Cloudflare Workers stack with D1, R2, Queues, and Durable Objects"
date: "2026-01-26"
dateModified: "2026-04-05"
excerpt: "Upload a PDF resume, get a live web portfolio. I built clickfolio.me entirely on Cloudflare's edge stack, D1 for data, R2 for files, Queues for async processing, Durable Objects for real-time WebSockets, and Gemini for AI parsing. Here's every technical decision, including the ones I regret."
tags:
  - Cloudflare Workers
  - D1
  - Durable Objects
...
published: true
featured: true
tldr: "clickfolio.me runs on Cloudflare's edge stack: D1, R2, Queues, Durable Objects, and Gemini. Claim-check uploads, hash dedup, and WebSocket status updates. It works, but D1's SQLite quirks mean you fight the platform as often as you build on it."
keyTakeaways:
...
howToSteps:
  - name: "Set up Cloudflare Workers with D1 and R2"
    text: "Create a Workers project with D1 database and R2 storage bindings. Configure Wrangler with queues, crons, and durable objects."
  - name: "Implement anonymous upload with Claim Check Pattern"
    text: "Allow users to upload PDFs before authentication. Store in R2 with a temp key, then link to the user after login."
  - name: "Add AI parsing with two-stage validation"
    text: "Use Gemini Flash Lite for resume parsing. Apply lenient transforms to AI output and strict Zod validation to user edits."
  - name: "Deploy real-time updates with WebSockets"
    text: "Replace HTTP polling with Durable Objects and WebSocket Hibernation. The DO evicts from memory when idle, keeping costs near zero."
...
That was the pitch I told myself. The reality is I wanted to see if I could build a production app using _only_ Cloudflare services, D1, R2, Queues, Durable Objects, Workers, without reaching for Supabase or Postgres or any external dependency. It's the same instinct behind [self-hosting my own logging platform](/blog/logwell-self-hosted-logging-platform) instead of paying for a SaaS — build the thing, learn the constraints. [clickfolio.me](https://clickfolio.me) is the result: upload a PDF resume, AI parses it, you get a customizable web portfolio with a public URL. The entire stack costs $0 per month — the same [zero-cost Cloudflare Workers hosting](/blog/zero-cost-portfolio-cloudflare-workers) that powers my own portfolio, just stretched across a full dynamic app. D1, R2, Queues, and Durable Objects all stay within free tier limits even at 500+ monthly active users.
...
The file hash deduplication saved me real money, roughly $0.02 per Gemini API call avoided, which added up to over $50 in savings during the first 3 months. Gemini API calls aren't free, and users upload the same resume multiple times more often than you'd think, fixing a typo in their name, trying different templates, abandoning the wizard and restarting. Same SHA-256 hash? Skip the parse.

But there's a subtler case: what if the same file is currently _being_ parsed? That's the `waiting_for_cache` status. Instead of firing a duplicate Gemini call, the new resume record waits for the first parse to complete, then copies the result. Ten-minute timeout if the primary parse dies. Belt and suspenders.
...
**Stage 2 (user edits): Strict Zod validation with XSS sanitization.** When users modify their resume content in the editor, _that's_ when I sanitize. Strict email regex, URL validation, HTML entity encoding. This is the trust boundary — AI output is internal data, user edits are untrusted input.
...
The initial version polled `GET /api/resume/status` every 3 seconds while the AI was parsing. It worked. It also hammered D1 with unnecessary reads and felt sluggish. Three seconds is an eternity when you're staring at a spinner.

I replaced it with Durable Objects and WebSocket Hibernation. The [commit message](https://github.com/divkix/clickfolio.me) says `feat(realtime): replace HTTP polling with Durable Objects + WebSocket Hibernation` and that undersells how much this changed.

Here's how it works:

The Durable Object is keyed by resumeId. The client connects via `GET /ws/resume-status?resume_id=X` with WebSocket upgrade. The DO accepts the connection using the Hibernation API. If there's a cached status in DO storage, it sends immediately. When the queue consumer finishes parsing, it POSTs to the DO. The DO broadcasts the status to all connected WebSocket clients. On terminal states (completed/failed), it schedules a 30-second alarm to clean up.

Hibernation API matters because without it, the Durable Object stays in memory for the entire parsing duration (30-40 seconds). With Hibernation, it evicts from memory when idle and only wakes up when a message arrives or a WebSocket connects. Cost: effectively zero for intermittent status updates.
...
I've been vocal about [using Supabase for side projects](/blog/side-project-stack-2026-grad-student/). clickfolio.me is the first project where I went all-in on D1 instead. Here's the honest assessment.

Drizzle ORM handles most SQLite weirdness transparently. Reads are fast with globally distributed replicas, setup requires zero config since it's a Wrangler binding with no connection strings, and the free tier is generous for side projects.

But there are sharp edges.

JSON is TEXT. There's no JSON column type, so you store JSON as a string and parse it on read. Drizzle's `json()` type with mode annotation helps, but you'll still write `JSON.parse()` in places where the ORM doesn't handle it, like raw SQL queries or when reading from Better Auth's user table where you added custom fields.
...
Date handling uses strings. No DATE type. Everything is ISO 8601 TEXT. Sorting works because ISO 8601 is lexicographically sortable, but date math requires parsing.
...
\*\*IP-based rate limiting for anonymous endpoints uses hashed IPs stored in `uploadRateLimits` with `expiresAt` timestamps. A single SQL query with conditional aggregation checks both hourly and daily limits in one roundtrip:
...
User-based rate limiting for authenticated endpoints queries existing records. Resume uploads: 5 per 24 hours (count from `resumes.createdAt`). Handle changes: 3 per 24 hours (count from `handleChanges`). Simple, no extra table needed.
...
I'd use Postgres (Neon or Supabase) instead of D1. D1 is fine for simple CRUD. The moment you need JSON queries, complex transactions, or reliable row-level security, you're fighting SQLite semantics on every query. The global distribution isn't worth those limitations for this use case.

I'd also start with WebSockets, not polling. The polling-to-Durable-Objects migration was painful. If you know you'll need real-time updates, design for it from day one. The DO Hibernation API is cheap enough that there's no reason to start with polling.

And fewer templates at launch. Seven meant seven components to keep responsive and consistent. Three would have been enough to validate the concept.

## What Stuck

Cloudflare's edge stack is production-viable, but you're an early adopter. The docs are sparse for complex patterns (Queues + DO + D1 in one Worker). Expect to read source code and Discord threads.

The Claim Check Pattern improved conversion by 43%. Reducing auth friction at the upload step, letting users see their resume being processed before they even sign up, meant more completions than hitting a login wall first.

> **Statistic:** The Claim Check Pattern improved conversion by 43%.

Deduplication saved more than I expected. The file hash cache and waiting-for-cache pattern eliminated roughly 30% of Gemini API calls in the first month. Users reupload constantly, same resume, different template, fixing a typo.

> **Statistic:** File hash deduplication eliminated roughly 30% of Gemini API calls in the first month.

Filtering at fetch time rather than storage time is the right call. Storing full data and filtering on read is simpler, more flexible, and much easier to reason about than trying to maintain multiple filtered copies.

Each edge computing constraint, no filesystem, no native modules, SQLite instead of Postgres, no ISR, middleware limitations, is manageable on its own. Stacked together, they add up to a fundamentally different development experience. Know what you're getting into before you're three sprints deep.

## Database Comparison: D1 vs Supabase vs Self-Hosted Postgres

| Feature                | D1 (Clickfolio)        | Supabase (PickMyClass)     | Self-Hosted Postgres (Alita)  |
| ---------------------- | ---------------------- | -------------------------- | ----------------------------- |
| **Type**               | SQLite at edge         | Managed Postgres           | Full Postgres                 |
| **JSON Support**       | TEXT (JSON.parse)      | Native JSONB               | Native JSONB                  |
| **Row-Level Security** | No                     | Yes                        | Yes (manual)                  |
| **Boolean Handling**   | Integers (0/1)         | Native boolean             | Native boolean                |
| **Cost**               | $0 free tier           | $0 free tier               | $0 (Oracle VPS)               |
| **Ops Work**           | None                   | None                       | Backups, upgrades, monitoring |
| **Best For**           | Simple CRUD on Workers | Real apps needing Postgres | Full control, 24/7 bots       |

D1 works for clickfolio because the data model is simple and write volume is low. For anything needing real Postgres features, use Supabase or self-host.

If you want to try it: [clickfolio.me](https://clickfolio.me). Upload a PDF, pick a template, get a URL. No code required.

For a broader look at when this serverless setup beats a traditional VPS, I wrote up the [real cost trade-offs between Cloudflare Workers and Docker](/blog/serverless-vs-traditional-servers-cloudflare-workers-docker) across 30+ projects.

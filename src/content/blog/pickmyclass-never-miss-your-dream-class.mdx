...
coverAlt: "ASU class enrollment notification system architecture diagram"
date: "2025-10-28"
...
howToSteps:
  - name: "Deploy the Cloudflare Worker and Supabase"
    text: "Set up a Cloudflare Worker for edge logic and Supabase for auth, database, and real-time subscriptions."
  - name: "Configure the Puppeteer scraper on Oracle Cloud"
    text: "Run a Puppeteer-based Chrome scraper on Oracle Cloud's free tier to fetch JavaScript-rendered class data from ASU."
  - name: "Set up email notifications with Resend"
    text: "Integrate the Resend API to send batch emails when class seats open or instructor assignments change."
  - name: "Monitor and optimize database queries"
    text: "Use batch queries and proper indexes to reduce database load from 30,000 queries per run to under 12."
sources:
...
The architecture keeps costs low: Cloudflare handles edge logic for free, Oracle's always-free tier runs the heavy Puppeteer work, and Supabase's generous free tier stores user data. [Building full-stack apps on Cloudflare Workers](/blog/clickfolio-full-stack-cloudflare-workers) taught me how powerful this edge-first architecture can be.
...
const classData = await page.evaluate(() => {
  ...
  return Array.from(rows).map((row) => ({
    ...
    instructor:
      row.querySelector(".instructor-name")?.textContent?.trim() || "Staff",
  }));
});
```
...
Next.js 15 for the frontend, Cloudflare Workers for edge logic, Supabase for auth and real-time subscriptions, PostgreSQL with row-level security, Puppeteer for scraping, and Resend for emails. It's a subset of the [broader stack I use for every side project as a grad student](/blog/side-project-stack-2026-grad-student).
...
[PickMyClass](https://pickmyclass.app) is running. 10,000+ users, $34/month, no serious outages. It's not [300K+ users on a Telegram bot](/blog/scaling-telegram-bot-300k-users), but it's real people relying on something I built.

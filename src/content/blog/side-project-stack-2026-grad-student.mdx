---
title: "My 2026 Stack for Shipping 30+ Side Projects After Grad School"
seoTitle: "My 2026 Stack for Shipping 30+ Side Projects (MS CS, 300K+ Users)"
seoDescription: "I shipped 30+ projects while finishing my MS at ASU. Next.js, Supabase, Cloudflare — real lessons from tools serving 300K+ users."
coverAlt: "Tech stack diagram showing Next.js, Supabase, and Cloudflare Workers integration"
date: "2025-11-24"
dateModified: "2026-05-20"
excerpt: "I shipped 30+ side projects while finishing my MS in Computer Science at ASU. Here's the stack that let me build fast, deploy faster, and still sleep."
tags: ["Side Projects", "Tech Stack", "Next.js", "Supabase", "Cloudflare", "TypeScript", "Building in Public", "Productivity", "2026"]
published: true
tldr: "Next.js 15, TypeScript, Supabase, and Cloudflare Workers. Managed services over self-hosting, copy-paste UI over custom design, deploy in 30 seconds. Built for a student schedule, not a resume."
keyTakeaways:
  - "Supabase and Cloudflare eliminate ops work — your time beats saving $5/month"
  - "TypeScript catches bugs before users do; non-negotiable for projects you revisit months later"
  - "shadcn/ui beats building components from scratch every time"
  - "Cloudflare Workers deploy in ~30 seconds and stay on the free tier for most side projects"
  - "Ship fast, refactor later — architectural purity can wait until something has users"
sources:
...
faq:
  ...
  - q: "What about costs at scale?"
    a: "PickMyClass serves thousands of students on Cloudflare's free tier. Alita Robot (300K+ users) runs on minimal infra. Most projects never hit paid tiers."
  - q: "Why Biome over ESLint + Prettier?"
    a: "20x faster, one config file, zero conflicts between linter and formatter. Migrated all my projects in 2024 and never looked back."
---

I've shipped 30+ projects while getting my master's degree in Computer Science at ASU. None of them are perfect. Most serve specific needs. A few have actually scaled to real users—[PickMyClass](/blog/pickmyclass-never-miss-your-dream-class) helps thousands of ASU students grab classes, Alita Robot serves 300K+ Telegram users.
...
My portfolio site (the one you're reading this on) serves global traffic on Cloudflare's free tier — I wrote about [how it runs at zero cost on Workers](/blog/zero-cost-portfolio-cloudflare-workers). Zero cold starts. Zero downtime in 6 months.
...
Alita is a [Telegram bot serving 300K+ users](/blog/scaling-telegram-bot-300k-users). I wrote it in Go, not TypeScript.
...
## Lessons from 300K+ Users

Self-hosting Postgres in Docker taught me that managed services save more time than they cost. I was spending weekends debugging "why is this container restarting" instead of building features. Supabase is free for most projects and handles everything I used to babysit. If you're optimizing cloud costs before you have real users, you're optimizing the wrong thing.

TypeScript catches half my bugs before they hit production. When users actually depend on your project — students trying to get into a class during registration week — silent runtime failures are not acceptable. Types are not optional at that point.

Deployment friction is a real project killer. If deploying takes more than 5 minutes, you do it less often. Less frequent deploys means bigger diffs, bigger diffs means more bugs. Cloudflare Workers and Vercel make deployment boring, which is exactly right.

PickMyClass users care that they get notified when CS 101 has an open seat. They don't care whether it's Next.js or Rails. That sounds obvious until you've spent a week evaluating frameworks instead of shipping.

The scattered dashboard problem hit me later than it should have: Cloudflare observability, Supabase logs, journalctl on the VPS — three different places to look when something breaks at 3 AM. I [built my own logging platform](/blog/logwell-self-hosted-logging-platform/) to fix that. One search across all my projects.

## The Stack Decisions I Regret

MongoDB on early projects — I thought schemaless meant flexibility. It meant chaos. Every query was a gamble. Migrating to Postgres fixed it, but it was painful. Schemas are good actually.

Not starting with TypeScript. I have 5 projects in plain JavaScript that I'm scared to touch now. Adding types retroactively is miserable. Just start with TypeScript.

Over-engineering before anyone used it. I built caching layers and microservices before I had a single real user. Wasted weeks. Start with a monolith. Scale when you actually need to, which is probably never.

Fighting vendor lock-in harder than it deserved. I spent days setting up self-hosted auth instead of using Supabase Auth. The self-hosted version broke three times. Supabase Auth has worked without a problem for two years. Pragmatism beats purity.
...
Use managed services aggressively. Database, auth, file storage, email—all solvable with Supabase, Cloudflare, and Resend. Don't self-host unless you're learning DevOps specifically — or [building your own observability tools](/blog/logwell-self-hosted-logging-platform) because nothing existing fits your needs.

...
Around the same time I was maintaining Alita on a VPS, I started building Clickfolio — [a full-stack portfolio builder running entirely on Cloudflare Workers](/blog/clickfolio-full-stack-cloudflare-workers). I picked Workers for the entire backend.
...
This is my default stack recommendation for 2026: Cloudflare Workers + Supabase. Fall back to self-hosted Docker only when the app needs to run continuously or has heavy database requirements that outgrow managed tiers. I've written more about the broader [side-project stack I use to ship while in grad school](/blog/side-project-stack-2026-grad-student), if you want the full picture.
...
Workers alone works when your write volume is low, you don't need real Postgres features, and you want push-and-forget deployment with zero maintenance. The free tier handles 100K requests per day and 5M D1 row reads — generous enough that most side projects never pay. My own portfolio runs this way at [$0/month on Cloudflare Workers](/blog/zero-cost-portfolio-cloudflare-workers), and it's never come close to the limit.
